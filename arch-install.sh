...
function chroot_config
{
	...
	cd "$mhome" && for pkg in "${pkgs[@]}"; do
		mkdir $pkg && cp "$repo/$pkg" "$pkg/PKGBUILD"
       	done && cd "$repo" && cp -r .vim* "$mhome"
...
passwd "$user" && pkgs=('vim-git' 'dwm-git' 'st-git' 'dmenu-git')
pacman -Syyuu && chown -R "$user":users "$mhome" && cd "$mhome"
for pkg in \${pkgs[@]} ; do cd \$pkg && runuser -u "$user" -- makepkg -sri
cd "$mhome" && mv \$pkg /usr/src/ && ; done
cd "$mhome" && printf "$brc" > .bashrc && printf "$bprc" > .bash_profile
git clone https://aur.archlinux.org/yay.git && chown -R "$user:users" yay
cd yay && runuser -u "$user" -- makepkg -sri
runuser -u "$user" -- yay -Y --gendb
runuser -u "$user" -- yay -Syu --devel
